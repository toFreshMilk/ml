...
for C, marker in zip([0.001, 1, 100], ['o', '^', 'v']):
    lr_model_l1 = LogisticRegression(solver='liblinear', 
                                     C=C, penalty="l1", 
                                     max_iter=1000).fit(X_train, y_train)
    print("C={:.3f} 인 l1 로지스틱 회귀의 훈련 정확도: {:.2f}".format(
          C, lr_model_l1.score(X_train, y_train)))
    print("C={:.3f} 인 l1 로지스틱 회귀의 테스트 정확도: {:.2f}".format(
          C, lr_model_l1.score(X_test, y_test)))
    
    plt.plot(lr_model_l1.coef_.T, marker, label="C={:.3f}".format(C))
